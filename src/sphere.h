...
private:
	...
	/**
	 * @brief 		Функция sphere::get_sphere_uv(...) рассчитывает uv-координаты на текстурной карте
	 * 				при помощи параметаризованного представления сферы: (u,v) = (phi/2*pi, theta/pi), где 
	 * 				phi -- улгол от -x до +z, от -z до +z (т.е. угол вокруг оси Y), theta -- угол от нижнего
	 * 				полюса.
	 * 
	 * @param[in] 	p -- вектор до точки пересечения луча и сферы непосредственно на самом луче.
	 * @param[in] 	u -- x-координа на изображении текстурной карты.
	 * @param[in] 	v -- y-координа на изображении текстурной карты.
	 * 
	 * @details		Для нахождения координат (u,v) требуется в первую очередь определить углы (theta, phi) через 
